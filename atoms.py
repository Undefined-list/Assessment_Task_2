...
def Ballence(Positive, Negative):
    Negativecharge = Negative.charge * -1
    ...
    if Negativecharge == Positive.charge:
        return f"{Positive.symbol} {Negative.symbol}"
        ...
        if factor == 0:
            ...
            return f"{Positive.symbol} {int(divisor)} {Negative.symbol}"
        else:
            for x in range(1, 5):
                for y in range(1, 5):
